  ...
  return (
    <div >
      ...
      <div className='qASession' >
        ...
        <p >Answer: useState is a Hook (function) that allows  to have state variables in functional components. For that, we have to pass the initial state to this function and it returns a variable with the current state value (not necessarily the initial state) and another function to update this value. Therefpre, we have to create a functional component and throw some React hook at it that tracks state, can also update it, and it just works.</p>
      </div>
    </div>
  );
}
